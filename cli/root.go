// Package cli assembles the swapi command tree from the swapi
...
// NewApp assembles the kit application from the swapi domain. The
...
//
// To add a command, declare it in swapi/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := swapi.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(swapi.Domain{}).Register(app)
