...
describes what users get. Pick a format with `-o`, or let swapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
swapi <command> -o table   # aligned columns for reading
swapi <command> -o jsonl   # one JSON object per line, for piping
swapi <command> -o json    # a single JSON array
swapi <command> -o csv     # spreadsheet friendly
swapi <command> -o tsv     # tab-separated
swapi <command> -o url     # just the URL column
swapi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
swapi <command> --fields id,title,url
```
...
```bash
swapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
swapi <command>            # a table, because this is a terminal
swapi <command> | wc -l    # JSONL, because this is a pipe
```
